# update_issues_to_50k.py
import subprocess

issues_updates = [
    {
        "number": "4",
        "title": "🏆 [Bounty: 50,000 $ROBIN_MCP] Integrate Pyth / Chainlink Oracles for Real-Time Gold & Silver Valuation",
        "body": """### Objective
...
4. Include your wallet address in the PR body (e.g., `Wallet: 0x...`).
5. Submit a Pull Request. Once merged, the **50,000 $ROBIN_MCP** bounty will be sent directly to your wallet.
...
"""
    ...
    {
        "number": "5",
        "title": "🏆 [Bounty: 50,000 $ROBIN_MCP] Implement Native Token ($ROBIN_MCP) Staking & Yield Distribution Contract",
        "body": """### Objective
...
3. Include your wallet address in the PR body (e.g., `Wallet: 0x...`).
4. Submit a Pull Request. Once merged, the **50,000 $ROBIN_MCP** bounty is paid out instantly.
...
"""
    ...
    {
        "number": "6",
        "title": "🏆 [Bounty: 50,000 $ROBIN_MCP] Create a Telegram Mini-App for Launching Meme Coins & Tracking Trusts",
        "body": """### Objective
...
def edit_issue(number, title, body):
    ...
    if res.returncode == 0:
        print(f"✅ Updated Issue #{number} to 50k $ROBIN_MCP reward")
    else:
